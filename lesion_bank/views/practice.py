...
def practice_view(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            npToSql_uploads(niftiTo2d(uploaded_image.file_path.name), uploaded_image.upload_id, PracticeImageVoxels)
            npToSql_uploads(niftiTo2d(uploaded_image.true_file_path.name), uploaded_image.upload_id, TrueImageVoxels)
